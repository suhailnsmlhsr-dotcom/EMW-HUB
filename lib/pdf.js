...
function drawHeader(doc, marginL, marginR, pageW, label, sublabel) {
  let y = 22;
...
function drawItemsTable(doc, marginL, marginR, pageW, contentW, y, items) {
  ...
  doc.text('WORK ID', col1 + 3, y + rowH / 2 + 1.2);
  doc.text('DESCRIPTION', col2, y + rowH / 2 + 1.2);
  doc.text('DATE ASSIGNED', col3, y + rowH / 2 + 1.2);
  doc.text('AMOUNT (Rs.)', col4 - 3, y + rowH / 2 + 1.2, { align: 'right' });
  ...
  items.forEach((item) => {
    y += rowH;
    doc.text(item.workId || '-', col1 + 3, y - rowH / 2 + 1.5);
    doc.text(item.description || '-', col2, y - rowH / 2 + 1.5);
    doc.text(fmtDate(item.dateAssigned), col3, y - rowH / 2 + 1.5);
    doc.text(Number(item.amount || 0).toFixed(2), col4 - 3, y - rowH / 2 + 1.5, { align: 'right' });
...
export function generateInvoicePDF(document, { download = true } = {}) {
  ...
  let y = drawHeader(doc, marginL, marginR, pageW, 'INVOICE', document.status === 'paid' ? 'Paid' : 'Payment Due');
  ...
  const fileName = `${document.doc_number}_${(document.client_name || 'client').replace(/\s+/g, '_')}.pdf`;
...
export function generateReceiptPDF(document, { download = true } = {}) {
  ...
  const isPartial = (document.balance_due || 0) > 0;
  let y = drawHeader(doc, marginL, marginR, pageW, 'RECEIPT', isPartial ? 'Partial Payment' : 'Payment Received');
  ...
  const fileName = `${document.doc_number}_Receipt_${(document.client_name || 'client').replace(/\s+/g, '_')}.pdf`;
  if (download) doc.save(fileName);
  return doc;
}
